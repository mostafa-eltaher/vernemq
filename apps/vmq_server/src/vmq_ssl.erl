...
-spec ciphersuite_transform([string()]) -> [string()|binary()].
ciphersuite_transform([]) ->
    filter_ciphers(ciphers());
ciphersuite_transform(CiphersString) when is_list(CiphersString) ->
    %% parsing to binary list, this is needed since ssl_cipher:openssl_suite
    %% does not handle PSK cipher names.
    [openssl_psk_suite(C) || C <- string:tokens(CiphersString, ":")].
...
%% PSK ciphers supported by OpenSSL
openssl_psk_suite("PSK-AES256-CBC-SHA") ->
    ssl_cipher:suite({psk, aes_256_cbc,sha});
openssl_psk_suite("PSK-3DES-EDE-CBC-SHA") ->
    ssl_cipher:suite({psk, '3des_ede_cbc',sha});
openssl_psk_suite("PSK-AES128-CBC-SHA") ->
    ssl_cipher:suite({psk, aes_128_cbc,sha});
openssl_psk_suite("PSK-RC4-SHA") ->
    ssl_cipher:suite({psk, rc4_128,sha});
openssl_psk_suite(CipherString) ->
    ssl_cipher:openssl_suite(CipherString).
